...
#line 3 "..\\TASK\\gimbal_task.c"
#line 4 "..\\TASK\\gimbal_task.c"
#line 5 "..\\TASK\\gimbal_task.c"
#line 6 "..\\TASK\\gimbal_task.c"
...
{
	dmp_getdata();                         
	mpuspeed_pit = gyro_mpu[1]*(2000.0/65535.0); 
	mpuspeed_yaw = gyro_mpu[2]*(2000.0/65535.0); 
...
void gimablcheck_init(gimbal_checktask *gimbal_check_type, int add, int max, int min)
{
    gimbal_check_type->add = add;
    gimbal_check_type->max_angle = max;
    gimbal_check_type->min_angle = min;
    gimbal_check_type->out = 0;
}

void gimablcheck_calc(gimbal_checktask *gimbal_check_type, int input_angle, int exp_angle)
{
	 
    if (gimbal_check_type->exp_angle > gimbal_check_type->input_angle)
    {
        gimbal_check_type->out = gimbal_check_type->input_angle +  gimbal_check_type->add;
			  gimbal_check_type->input_angle = gimbal_check_type->out;
    }
    else if (gimbal_check_type->input_angle > gimbal_check_type->exp_angle)
    {
			  gimbal_check_type->out = gimbal_check_type->input_angle -  gimbal_check_type->add;
			  gimbal_check_type->input_angle = gimbal_check_type->out;
    }
		else if (gimbal_check_type->input_angle == gimbal_check_type->exp_angle)
    { gimbal_check_type->out = gimbal_check_type->input_angle;}
	 	
		if (gimbal_check_type->out > gimbal_check_type->max_angle)
    {
        gimbal_check_type->out = gimbal_check_type->max_angle;
    }
    else if (gimbal_check_type->out < gimbal_check_type->min_angle)
    {
        gimbal_check_type->out = gimbal_check_type->min_angle;
    }
...
void manual_control(void)
{
		...
		remote_yaw=(remote_control.ch1*0.05)+remote_yaw;     
    pid_calc(&check_yaw,motor_info[2].rotor_angle,remote_yaw); 
		
 		
		remote_pit=(remote_control.ch2*0.05)+remote_pit;
    pid_calc(&check_pitch,motor_info[1].rotor_angle,remote_pit);
	 

		remote_bodan=remote_control.ch3*5 ;
		pid_calc(&check_bodan ,motor_info[3].rotor_speed,remote_bodan);
	...
	if (fric_pwm.out > 1150)
	{
			fric_pwm.out = 1150;
	}
	else if (fric_pwm.out < 1000)
	{
			fric_pwm.out = 1000;
	}
...
void MPU6050_control(void)
{
		...
		if((remote_control.ch1<10)&&(remote_control.ch1>-10))
		{
			if(angle_YAW==0)  
			{
				angle_YAW=yaw;
			}
			pid_calc(&pid_yuntai_yaw,yaw,angle_YAW);
      speed_YAW=pid_yuntai_yaw.pos_out;
	  }
		else
		{
			angle_YAW=0;
			speed_YAW=-remote_control.ch1*80/660;     
		}
		pid_calc( &pid_yuntai_yaw_speed,mpuspeed_yaw,speed_YAW);
		
		
		 		
		if((remote_control.ch2<10)&&(remote_control.ch2>-10))
		{
			if(angle_PIT==0)  
			{
				angle_PIT=pit;
			}
			pid_calc( &pid_yuntai_pitch,pit,angle_PIT);
      speed_PIT=pid_yuntai_pitch.pos_out;
	  }
		else
		{
			angle_PIT=0;
			speed_PIT=-remote_control.ch2*150/660+50;
		}
		pid_calc( &pid_yuntai_pitch_speed,mpuspeed_pit,speed_PIT);	
   ...
   {	
	  remote_mocha=(int)(1000+remote_control.ch4*0.5);
		if(remote_mocha>1150){remote_mocha=1150;}
		if(remote_mocha<1000){remote_mocha=1000;}
	  fric1_on(remote_mocha);       
	  fric2_on(remote_mocha);
	 }
		
  	else if(remote_control.switch_right==3)
      {
				fric_off();
			  fric_pwm.out=1000;
			}
		...
		set_yuntaimotor_voltage(-pid_yuntai_pitch_speed.pos_out,-pid_yuntai_yaw_speed.pos_out,check_bodan.pos_out);
...
void moto_control(void)
{
    ...
    remote_speed=30;
	  pid_calc(&pid_yuntai_yaw_speed,mpuspeed_yaw,remote_speed ); 
	  set_yuntaimotor_voltage(0,pid_yuntai_yaw_speed.pos_out,0);
}
