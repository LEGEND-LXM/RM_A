...
enum lp_accel_rate_e {
    ...
    INV_LPA_20HZ,
    INV_LPA_40HZ
#line 374 "..\\MPU6050\\inv_mpu.c"
};

#line 417 "..\\MPU6050\\inv_mpu.c"

#line 425 "..\\MPU6050\\inv_mpu.c"

#line 453 "..\\MPU6050\\inv_mpu.c"
...
static int set_int_enable(unsigned char enable)
{
    ...
    if (st.chip_cfg.dmp_on) {
        ...
        if (MPU_Write_Len(st.hw->addr, st.reg->int_enable, 1, &tmp))
            return -1;
...
int mpu_get_gyro_reg(short *data, unsigned long *timestamp)
{
    ...
    if (MPU_Read_Len(st.hw->addr, st.reg->raw_gyro, 6, tmp))
        return -1;
...
int mpu_get_accel_reg(short *data, unsigned long *timestamp)
{
    ...
    if (MPU_Read_Len(st.hw->addr, st.reg->raw_accel, 6, tmp))
        return -1;
    data[0] = (tmp[0] << 8) | tmp[1];
    data[1] = (tmp[2] << 8) | tmp[3];
    data[2] = (tmp[4] << 8) | tmp[5];
...
int mpu_get_temperature(long *data, unsigned long *timestamp)
{
    ...
    if (MPU_Read_Len(st.hw->addr, st.reg->temp, 2, tmp))
        return -1;
    raw = (tmp[0] << 8) | tmp[1];
    if (timestamp)
        get_ms(timestamp);
...
int mpu_set_accel_bias(const long *accel_bias)
{
    ...
    if (MPU_Read_Len(st.hw->addr, 3, 3, data))
        return -1;
    fg[0] = ((data[0] >> 4) + 8) & 0xf;
    fg[1] = ((data[1] >> 4) + 8) & 0xf;
    fg[2] = ((data[2] >> 4) + 8) & 0xf;
    ...
    if (MPU_Read_Len(st.hw->addr, 0x06, 6, data))
        return -1;
    ...
    if (MPU_Write_Len(st.hw->addr, 0x06, 6, data))
        return -1;
    return 0;
}
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
    ...
    for (ii = 0; ii < packet_count; ii++) {
        ...
        if (MPU_Read_Len(st.hw->addr, st.reg->fifo_r_w, (12), data))
            return -1;
