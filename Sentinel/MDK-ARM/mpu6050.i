...
{ 
  ...
  TIM_Delay_ms(100);
	...
	MPU_Set_Accel_Fsr(0);					
	MPU_Set_Rate(50);						
	MPU_Write_Byte(0X38,0X00);	
	MPU_Write_Byte(0X6A,0X00);	
	MPU_Write_Byte(0X23,0X00);	
	MPU_Write_Byte(0X37,0X80);	
	res=MPU_Read_Byte(0X75);
	if(res==0X68)
	{
		MPU_Write_Byte(0X6B,0X01);	
		MPU_Write_Byte(0X6C,0X00);	
		MPU_Set_Rate(50);						
 	}else return 1;
	return 0;
}
...
uint8_t MPU_Set_Gyro_Fsr(uint8_t fsr)
{
	return MPU_Write_Byte(0X1B,fsr<<3);
}
...
uint8_t MPU_Set_Accel_Fsr(uint8_t fsr)
{
	return MPU_Write_Byte(0X1C,fsr<<3);
}
...
uint8_t MPU_Set_LPF(uint16_t lpf)
{
	...
	return MPU_Write_Byte(0X1A,data);
}
...
{
    uint8_t buf[6],res;  
	res=MPU_Read_Len(0X68,0X43,6,buf);
	if(res==0)
	{
...
{
    uint8_t buf[6],res;  
	res=MPU_Read_Len(0X68,0X3B,6,buf);
	if(res==0)
	{
