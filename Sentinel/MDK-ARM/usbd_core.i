...
#line 22 "../Middlewares/ST/STM32_USB_Device_Library/Core/Src/usbd_core.c"
    ...
    if ( pdev->ep0_state == 0x03U)
    {
      if(pep->rem_length > pep->maxpacket)
      {
        ...
        USBD_CtlContinueRx (pdev,
                            pdata,
                            (uint16_t)(((pep->rem_length) < (pep->maxpacket)) ? (pep->rem_length) : (pep->maxpacket)));
      }
      else
      {
  ...
  if(epnum == 0U)
  {
    ...
    if ( pdev->ep0_state == 0x02U)
    {
      if(pep->rem_length > pep->maxpacket)
      {
        ...
        USBD_LL_PrepareReceive (pdev, 0U, 0, 0U);
      }
      else
      {  
        ...
        {
          USBD_CtlContinueSendData(pdev, 0, 0U);
