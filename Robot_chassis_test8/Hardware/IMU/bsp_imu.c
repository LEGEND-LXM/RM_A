...
#include "tim.h"		//mpu6500温度控制需要其中的 MPU6500_PWM_TEMPERATURE_SET 语句 现在不需要了
...
void mpu_get_data()
{
    ...
    imu.temp = 21 + mpu_data.temp / 333.87f;
		if(imu.temp<50)
		{
			HAL_GPIO_WritePin(GPIOB, GPIO_PIN_5, GPIO_PIN_SET);//设置imu电阻引脚为高电平
		}
		else
		{
			HAL_GPIO_WritePin(GPIOB, GPIO_PIN_5, GPIO_PIN_RESET);//设置imu电阻引脚为低电平
...
uint8_t mpu_device_init(void)
{
	...
	{
		while(robot_status.placement != ROBOT_STATIC)//等待机器人静止
		{
			...
			Place_Staus_Check(gyro_status);//检测机器人状态
		}
		mpu_offset_call();
	}
	return 0;
